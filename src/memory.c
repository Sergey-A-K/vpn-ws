...
void vpn_ws_peer_destroy(vpn_ws_peer *peer) {
	int fd = peer->fd;
	if (fd > -1) { vpn_ws_announce_peer(peer, "removing"); close(fd); }
	...
	while(macs) { vpn_ws_mac *next = macs->next; free(macs); macs = next; }
	free(peer);
	if (vpn_ws_conf.peers) vpn_ws_conf.peers[fd] = NULL;
}

void *vpn_ws_malloc(uint64_t amount) {
	void *ptr = malloc(amount);
	if (ptr == NULL) { vpn_ws_error("vpn_ws_malloc()/malloc()"); return NULL; }
	return ptr;
}

void *vpn_ws_calloc(uint64_t amount) {
    void *ptr = calloc(1, amount);
    if (ptr == NULL) { vpn_ws_error("vpn_ws_malloc()/calloc()"); return NULL; }
    return ptr;
}
